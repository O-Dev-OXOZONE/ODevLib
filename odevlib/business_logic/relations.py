from typing import Dict, List, Type, Union

from django.db import models
from django.db.models import ForeignObjectRel


def get_model_relations(model: Type[models.Model]) -> List[models.Field]:
    return [f for f in model._meta.get_fields()
            if isinstance(f, (models.ForeignKey, models.ManyToManyField, models.OneToOneField))]


def get_reverse_model_relations(model: models.Model) -> List[ForeignObjectRel]:
    """
    Returns reverse (i.e. autogenerated) foreign and many-to-many relations of a given model.
    Keep note that this list only includes relations which are not present in the code, but generated automatically by
    Django because other models refer to this model in its fields.
    """
    return [f for f in model._meta.get_fields() if isinstance(f, ForeignObjectRel)]


def get_relations(instance: models.Model):
    reverse_fks: List[ForeignObjectRel] = get_reverse_model_relations(instance)

    def get_item(f) -> Union[Dict, None]:
        try:
            # noinspection PyProtectedMember
            return {
                'model_name': f'{f.related_model._meta.app_label}.{f.related_model._meta.model_name}',
                'verbose_name': str(f.related_model._meta.verbose_name),
                'verbose_name_plural': str(f.related_model._meta.verbose_name_plural),
                'related_field': str(f.remote_field.verbose_name or f.remote_field.name),
                'ids': list(getattr(
                    instance,
                    f.related_name or f'{f.related_model._meta.model_name}_set'
                ).values_list('pk', flat=True)),
            }
        except Exception:
            # noinspection PyProtectedMember
            return {
                'model_name': f'{f.related_model._meta.app_label}.{f.related_model._meta.model_name}',
                'verbose_name': str(f.related_model._meta.verbose_name),
                'verbose_name_plural': str(f.related_model._meta.verbose_name_plural),
                'related_field': 'Failed to obtain related field',
                'ids': [],
            }

    result = [get_item(f) for f in reverse_fks]
    return result
